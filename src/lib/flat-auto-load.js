...
Loader.prototype.load = function () {
  ...
  var tree = new Tree(result, dir, files);
...
function Tree (result, root, children) {
  var path = null;
  var child = null;
  var i = 0;

  // ensure instance
  if (!(this instanceof Tree)) return new Tree(result, root, children);
  ...
  for (; i < children.length; ++i) void function (child) {
    ...
    if (isDirectory(fpath)) {
      Tree(result, fpath, readdir(fpath));
